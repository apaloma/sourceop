...
CUtlSymbolOldTable* CUtlSymbolOld::s_pSymbolTable = 0; 
bool CUtlSymbolOld::s_bAllowStaticSymbolTable = true;
...
void CUtlSymbolOld::Initialize()
{
	...
	{
		s_pSymbolTable = new CUtlSymbolOldTable;
...
public:
	...
	{
		delete CUtlSymbolOld::s_pSymbolTable;
		CUtlSymbolOld::s_pSymbolTable = NULL;
	}
};

static CCleanupUtlSymbolTable g_CleanupSymbolTable;

CUtlSymbolOldTable* CUtlSymbolOld::CurrTable()
{
	Initialize();
...
CUtlSymbolOld::CUtlSymbolOld( char const* pStr )
{
	m_Id = CurrTable()->AddString( pStr );
}

char const* CUtlSymbolOld::String( ) const
{
	return CurrTable()->String(m_Id);
}

void CUtlSymbolOld::DisableStaticSymbolTable()
{
...
bool CUtlSymbolOld::operator==( char const* pStr ) const
{
...
inline const char* CUtlSymbolOldTable::StringFromIndex( const CStringPoolIndex &index ) const
{
...
bool CUtlSymbolOldTable::SymLess( CStringPoolIndex const& i1, CStringPoolIndex const& i2 )
{
...
bool CUtlSymbolOldTable::SymLessi( CStringPoolIndex const& i1, CStringPoolIndex const& i2 )
{
	char const* str1 = (i1 == INVALID_STRING_INDEX) ? g_LessCtx.m_pUserString :
											g_LessCtx.m_pTable->StringFromIndex( i1 );
	char const* str2 = (i2 == INVALID_STRING_INDEX) ? g_LessCtx.m_pUserString :
...
CUtlSymbolOldTable::CUtlSymbolOldTable( int growSize, int initSize, bool caseInsensitive ) : 
	m_Lookup( growSize, initSize, caseInsensitive ? SymLessi : SymLess ), m_StringPools( 8 )
{
}

CUtlSymbolOldTable::~CUtlSymbolOldTable()
{
...
CUtlSymbolOld CUtlSymbolOldTable::Find( char const* pString )
{	
	if (!pString)
		return CUtlSymbolOld();
	...
	return CUtlSymbolOld( idx );
}


int CUtlSymbolOldTable::FindPoolWithSpace( int len )	const
{
...
CUtlSymbolOld CUtlSymbolOldTable::AddString( char const* pString )
{
	if (!pString) 
		return CUtlSymbolOld( UTL_INVAL_SYMBOL );

	CUtlSymbolOld id = Find( pString );
...
char const* CUtlSymbolOldTable::String( CUtlSymbolOld id ) const
{
	if (!id.IsValid()) 
		return "";
...
void CUtlSymbolOldTable::RemoveAll()
{
	m_Lookup.RemoveAll();
