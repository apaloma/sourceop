...
#ifndef UTLSYMBOLOLD_H
#define UTLSYMBOLOLD_H
...
public:
	...
	CUtlSymbolOld() : m_Id(UTL_INVAL_SYMBOL) {}
	CUtlSymbolOld( UtlSymId_t id ) : m_Id(id) {}
	CUtlSymbolOld( char const* pStr );
	CUtlSymbolOld( CUtlSymbolOld const& sym ) : m_Id(sym.m_Id) {}
	
	// operator=
	CUtlSymbolOld& operator=( CUtlSymbolOld const& src ) { m_Id = src.m_Id; return *this; }
	
	// operator==
	bool operator==( CUtlSymbolOld const& src ) const { return m_Id == src.m_Id; }
...
protected:
	...
	static CUtlSymbolOldTable* CurrTable();
		
	// The standard global symbol table
	static CUtlSymbolOldTable* s_pSymbolTable; 
...
public:
	// constructor, destructor
	CUtlSymbolOldTable( int growSize = 0, int initSize = 32, bool caseInsensitive = false );
	~CUtlSymbolOldTable();
	
	// Finds and/or creates a symbol based on the string
	CUtlSymbolOld AddString( char const* pString );

	// Finds the symbol for pString
	CUtlSymbolOld Find( char const* pString );
	
	// Look up the string associated with a particular symbol
	char const* String( CUtlSymbolOld id ) const;
...
protected:
	...
	CUtlRBTree<CStringPoolIndex, unsigned short>	m_Lookup;
